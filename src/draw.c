...
// Funcion creada para generar un Seed para obtener valor aleatorio por
// ejecucion
...
#define kWander 16
  ...
  for (i = 0; i < kNumLightningPts;
       i++) // Calculate an even spread of points between�
  {         // obelisk tips and the target point.
    leftLightningPts[i].x =
        (leftDeltaH * i) / (kNumLightningPts - 1) + kLeftObeliskH;
    leftLightningPts[i].y =
        (leftDeltaV * i) / (kNumLightningPts - 1) + kLeftObeliskV;
    rightLightningPts[i].x =
        (rightDeltaH * i) / (kNumLightningPts - 1) + kRightObeliskH;
    rightLightningPts[i].y =
        (rightDeltaV * i) / (kNumLightningPts - 1) + kRightObeliskV;
  }
...
void FlashObelisks(bool flashThem) {
  ...
  if (flashThem) // Draw them "inverted"
  {
    ...
    SDL_RenderCopy(app.renderer, back.texture, &backSrcRect, &backSrcRect);
...
void StrikeLightning(void) {
  ...
  for (i = 0; i < kNumLightningPts - 1; i++) // Draw left lightning bolt.
  {
    ...
    SDL_RenderDrawLine(app.renderer, leftLightningPts[i].x,
                       leftLightningPts[i].y, leftLightningPts[i + 1].x - 1,
                       leftLightningPts[i + 1].y);
    presentScene();
  }
  ...
  for (i = 0; i < kNumLightningPts - 1; i++) // Draw right lightning bolt.
  {
    ...
    SDL_RenderDrawLine(app.renderer, rightLightningPts[i].x,
                       rightLightningPts[i].y, rightLightningPts[i + 1].x - 1,
                       rightLightningPts[i + 1].y);
    presentScene();
  }
}
